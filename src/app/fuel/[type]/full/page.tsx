...
export default function FullPage({ params }: Props) {
    ...
    <div className="min-h-screen w-full bg-white flex items-center justify-center">
      <main className="w-full max-w-3xl px-6 py-12 text-center">
        ...
        <div className="flex justify-center gap-4">
          <Link href={`/fuel/${params.type}`} className="text-2xl px-6 py-3 rounded-md bg-slate-200 hover:bg-slate-300 text-slate-800">Atrás</Link>
          <Link href={`/fuel/${params.type}/pago?mode=full`} className="text-2xl px-6 py-3 rounded-md bg-emerald-700 hover:bg-emerald-600 text-white">Proceder a pago</Link>
        </div>
      </main>
    </div>
  );
